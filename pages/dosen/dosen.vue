<template>
  <!-- <div class="container mx-auto mt-8">
    <div class="relative overflow-x-auto">
      <button
        type="button"
        class="text-white bg-green-500 hover:bg-green-600 focus:outline-none focus:ring-4 focus:ring-green-100 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"
      >
        <NuxtLink to="/matakuliah/CreateData">Tambah Data</NuxtLink>
      </button>
      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-sky-500 dark:bg-gray-700 dark:text-gray-400">
          <tr>
        ...
        <tbody>
          <tr class="bg-white dark:bg-gray-800" v-for="mk in Matakuliah" :key="mk.id">
  ...
  <div class="relative overflow-x-auto px-80 pt-5">
    <Header></Header>

    <NuxtLink to="/dosen/CreateData" class="flex justify-end items-center">
    ...
    <table class="w-full text-sm text-left text-blue-500 dark:text-gray-400 mt-5">
      ...
      <tbody>
        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700" v-for="ds in dosen" :key="ds.id">
          <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ ds.nama_dosen }}</th>
          <td class="px-6 py-3">{{ ds.gelar }}</td>
          <td class="px-6 py-3">{{ ds.prodi }}</td>
          <td class="px-6 py-3"><a href="#" class="font-medium text-red-600 dark:text-blue-500 hover:underline">Edit</a></td>
        </tr>
      </tbody>
    </table>
  </div>
...
const { data: dosen } = await useFetch('https://64abc8e39edb4181202e87b6.mockapi.io/akademik/v1/matakuliah/1/dosen');
</script>
